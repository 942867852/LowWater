...
## 十一、GDD 全集闭合（S8 / S9 与收尾裁决）

**至此 Phase 2 的 GDD 全集为 10 份，共 4772 行**（S0–S9）。

| 新增 | 篇幅 | 判定 |
|---|---|---|
| `systems/08-world-content.md` | 355 行 | **PASS** |
| `systems/09-presentation-ux.md` | 606 行 | **PASS** |

### 11.1 S8 的关键贡献

- **S3 的 `c_min` 首次有了真实来源**：主路河谷→井窖 **2200m / 27min**，必经旧政权配水总站，穿越 **2 个 `hazard ≥ 1` 遭遇点**，`LOSS_RATE = 0.25` → `C_trip ≈ 1.90 RU`（往返）→ **`c_min ≈ 0.059 RU/L`**。`p_min = 0.06 > 0` 来自**不可绕过的结构性风险**，玩法无法消除——这正是 D3「套利收益收敛但永不归零」的数学前置。
- 21 个容器的实际播种表（含三档 DC、`isUniqueAnchor`）；3 件唯一件按知识轴划分候选集（`|K|=3`、跨 ≥3 zone、三集互斥），三态 `SEALED→IN_TRANSIT→CONSUMED|DESTROYED|LOST`。
- **史线 = 10 片空间证据**，4 条可闭合 claim + **1 条永久留白**（`claim.final_decision`：决策层"下令弃守"还是"指挥真空"，最后一页文件永不补）。
- 两社区的空间依赖写在地形与路况里：河谷在低洼冲积地产水不产粮、井窖在黄土台地产药产种却致命缺水。

### 11.2 S8/S9 裁决（回填完成）

| 裁决 | 内容 |
|---|---|
| **37** | 🔴 **`successorActorId` 一人双岗已修正（阻塞级）**。原 `water_02` 同时是程九与石根的继承人——一个 actor 不能继承两个岗位，两人同日死亡时日切内会两次解析同一 `postId`，**继承结果依赖应用顺序，违反 S0 确定性纪律**（同 seed 必须逐位可复现），并会让 S7 岗位满员判定出现"一人占两岗"矛盾态。<br>**修法**：三条记录两两互异——`mend_01(程九) → patrol_01`（河谷 mend 岗 `rated==1` 无同岗次席，取同社区备位者，**能力缺口＝刻意保留的 P4 叙事材料**，知识由程九遗稿承接）、`care_01(苗青) → jing_cell.mend_01`、`water_01(石根) → water_02`。新增 `--assert-successor-injective` 与"双死顺序无关"验收 |
| **38** | **`C_trip` 采用往返口径**（`c_min ≈ 0.059`）。单程会系统性低估玩家实际成本，使 D3 套利收益显得比实际更差，误导决策 |
| **39** | S8 三个 Proposal 常量 + `protectedActors` 名单 + `LocationId` / `KnowledgeId` 命名规范 **批准登记进 S0 §D**（唯一定义处） |
| **40** | **S9 HUD 时钟不计入"UI 文字提示"**，但盲测必须同时跑**"关闭时钟"对照组**校准 C-1（昼夜色温 + 三个世界标点）权重；若关时钟组命中率跌破 70%，C-1 权重须上调。已写成验收 A13 |
| **41** | S9 其余：生理四值默认 SEGMENT 分段条 ✅ / 日志并列显示两版失真话 ✅（呈现"听来的事实"而非"答案"，不触 U-A）/ `BoardNote` 归纯本地存档 ✅（与 S7「写入量 = 0」一致）/ 三处口径统一 ✅（urgency 红标降冗余、`SCARCE` 与 `TIGHT` 拆两灯、FOOD 不可交易） |
| **42** | **`divIntM` 保持向零截断**，不并入对称舍入——C++ 整除本身即对称（`a/b == -(-a/b)`），改它反而引入错误。已在 ADR-005 补记 D1-b：语义显式声明为"向零截断"且**禁止用于精度敏感路径**（价格递推、账目结算、概率权重换算） |

### 11.3 S9 为验收 2 提供的四条必要条件通道

Core 验收 2（不看 UI 文字、两日后预测 NPC 位置三选三命中 ≥70%）在 S9 中被拆成可检验的通道：**C-1 时间锚**（昼夜色温四档 + 三个世界标点，06:00/12:00/20:00 恰落三档）、**C-2 岗位可辨**（无底盘=人 + 岗位 glyph 名签，同一 NPC 形状两日完全一致）、**C-3 路线可读**（固定通勤路线 + 踩实旧径 + 岔口脚印，偏离**只**在变化预告时发生）、**C-4 携带物剪影**（桶/灯/工具/袋，8m 完整 / 25m 剪影+步频 / 60m 静止·移动·有灯）。

> **S9 的关键设计立场**：HUD 不做任何"帮玩家理解"的提示、注解、箭头——**理解必须来自归纳**。§6.3 列了 15 条**刻意不显示**的 HUD 信息（urgency 数字及一切可数化替身、全图任务列表/导航箭头、全局声望条、24h 作息甘特、NPC 需求数值、位置标记、归因链……）。原则：**凡是关于世界的答案，HUD 一律不给；凡是玩家自身状态与时间，给最小集。**

### 11.4 Phase 2 最终判定

> ## **PASS**

10 份 GDD 全部交付并通过一致性评审，40+ 条跨文档冲突全部裁决回填，9 道闸门全过，无遗留阻塞项。原 CONCERNS 的唯一理由（`docs/architecture/` 未建）已在 Phase 3 解除。

**已知风险与缓解**（阶段切换惯例）：
1. **R1 LOD 连续量分叉致 save/load 差异 >5%** → 已由 tick 分级 + 边界 tick 补做判定（ADR-003，A-3 PASS）缓解，但需 S1/S2 实装后实测。
2. **R2 量纲错误未签收** → 已修正（裁决 27）并回填，`build.sh` 需加断言防回流。
3. **R3 日切批处理中断链致 `UNATTRIBUTED > 0`** → S0 §C3 已定"写不出归因链的链条视为未实现"，控制清单已列验证手段。
4. **R4 S8/S9 篇幅偏大（355 / 606 行）** → 判例保留（强制项不可压缩），但**后续任何新增文档不得超过 350 行**。
